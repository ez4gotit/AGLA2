...
void preyPredator(double v0, double k0, double a1, double b1, double a2, double b2, int T, int N )
{

    double vt = a1 * v0 - b1 * v0 * k0;
    double kt= a2* k0+ b2*v0*k0;
    for (int i = 0; i < T; i++)
    {
        vt = a1 * vt - b1 * vt * kt;
        kt = -a2 * kt + b2 * vt * kt;
...
int main() {
    ...
    for (double t = 0; t <= T;)
    {
        ...
        vt = (v0 - a2 / b2) * cos(sqrt(a1 * a2) * t) - ((k0 - a1 / b1) * sqrt(a2) * b1 * sin(sqrt(a1 * a2) * t) / (b2 * sqrt(a1)));
        vs.push_back(vt);
        double Vt = vt + a2 / b2;
        cout << Vt << " ";
        t += dt;
    ...
    for (double t = 0; t <= T;)
    {
        kt = ((v0 - a2 / b2) * sqrt( a1 ) * b2 * sin( sqrt( a1 * a2 ) * t )) / (b1 * sqrt( a2 )) + (k0 - a1 / b1) * cos( sqrt( a1 * a2 ) * t);
        ks.push_back(kt);
        double Kt = kt + a1 / b1;
        cout << Kt << " ";
        t += dt;
